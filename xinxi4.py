...
import unit
from api.sk4 import xintiao, zhuce, CCPTX_Report, CCPRX_Report, BLINK_Report
...
def Recv_info(ms):
    ...
    try:
        if ms == b'':
            ...
            print("配置基站4定位参数：", hex(ms[0]), hex(ms[1]), hex(ms[2]))
        elif ms[0] == 0x44:
            ...
            print("配置基站4天线延迟参数：", hex(ms[0]))
        else:
            print(" 4其他参数：", hex(ms[0]))
            ...
            if X != Blink_seq:
                ...
                try:
                    ...
                    for Tag_Addr in json1:
                        tt = time1 + cou.BINK(Tag_Addr[1][0], Tag_Addr[1][1], 4) - cou.BINK(
